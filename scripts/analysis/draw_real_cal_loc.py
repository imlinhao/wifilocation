...
def get_real_cal_loc_list(query_list, db):
	...
	for real_loc in real_loc_list:
		...
		real_cal_loc_list.append([real_loc, cal_loc])
	return real_cal_loc_list
...
def get_3channel_move_query_list(filename, movetime_list, loc_list):
	...
	while True:
		...
		ap_rssi_all = []
		for freq in ["2412","2437","2462"]:
			ap_rssi_findall = re.findall(freq+',(..:..:..:..:..:..),.*?(-\d*);',line)
			ap_rssi_all.extend(ap_rssi_findall)
		time_ap_rssi_all_list.append([time, ap_rssi_all])
	print(time_ap_rssi_all_list)
	for i in range(0, len(loc_list)):
		time_count = len(time_ap_rssi_all_list)
		j = 0
		query_entry = {}
		while ( j < time_count):
			j = j+1
			if (j == time_count and time_ap_rssi_all_list[j-1][0] <= movetime_list[i]): 
				ap_rssi_all = time_ap_rssi_all_list[j-1][1]
				for ap,rssi in ap_rssi_all:
					query_entry[ap] = int(rssi)
				query_list.append(query_entry)
				break
			if(j == time_count): break
			if (time_ap_rssi_all_list[j][0] > movetime_list[i]):
				ap_rssi_all = time_ap_rssi_all_list[j-1][1]
				for ap,rssi in ap_rssi_all:
					query_entry[ap] = int(rssi)
				query_list.append(query_entry)
...
if __name__ == '__main__':
	...
	#print("still qurey list [1,7)"); print(query_list[1:7])
	allchannel_still_real_cal_loc_list = get_real_cal_loc_list(query_list, db)
	plt_real_cal_loc_list(allchannel_still_real_cal_loc_list, 0.2, 0.8)
	...
	#print("move qurey list [1,7)"); print(query_list[1:7])
