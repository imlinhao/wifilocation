...
def plt_cdf(err_distance_list, color_style):
	...
	cdf = np.cumsum(pdf)
	print(cdf)
	p = plt.plot(bins,np.append([zero_count_percentage],cdf),color_style)
	return p

if __name__ == '__main__':
	DATA_ROOT = "../datasets/fingerprint_db/liang_mi"
	ALLCHANNEL_OFFLINE_STILLTIME_FN = DATA_ROOT+"/2013_12_4_21_42_49/stilltime.txt" 
	ALLCHANNEL_OFFLINE_DB_FN = DATA_ROOT+"/2013_12_4_21_42_49/wifi.txt"
	ALLCHANNEL_ONLINE_MOVETIME_FN = DATA_ROOT+"/2013_12_4_21_53_6/stilltime.txt"
	ALLCHANNEL_ONLINE_QUERY_FN = DATA_ROOT+"/2013_12_4_21_53_6/wifi.txt"

	THREECHANNEL_OFFLINE_STILLTIME_FN = DATA_ROOT+"/2013_12_4_21_42_49/stilltime.txt" 
	THREECHANNEL_OFFLINE_DB_FN = DATA_ROOT+"/2013_12_4_21_42_49/wifi.txt"
	THREECHANNEL_ONLINE_MOVETIME_FN = DATA_ROOT+"/2013_12_4_21_37_52/movetime.txt"
	THREECHANNEL_ONLINE_QUERY_FN = DATA_ROOT+"/2013_12_4_21_37_52/getscanJNI.txt"
	loc_list = range(0,51)

	fig, ax = plt.subplots()
	color_style = ["r*-","g+-","bo-","k*--","r+--","go--","b*-."]

	stilltime_list = get_stilltime_list(ALLCHANNEL_OFFLINE_STILLTIME_FN)
	db = construct_allchannel_db(ALLCHANNEL_OFFLINE_DB_FN, stilltime_list)
	online_movetime_list = get_movetime_list(ALLCHANNEL_ONLINE_MOVETIME_FN)
	query_list = get_allchannel_move_query_list(ALLCHANNEL_ONLINE_QUERY_FN, online_movetime_list, loc_list)
	err_distance_list = get_err_distance_list(query_list, db)
	plt_cdf(err_distance_list, color_style[0])

	threechannel_stilltime_list = get_stilltime_list(THREECHANNEL_OFFLINE_STILLTIME_FN)
	threechannel_db = construct_3channel_db(THREECHANNEL_OFFLINE_DB_FN, threechannel_stilltime_list)
	threechannel_online_movetime_list = get_stilltime_list(THREECHANNEL_ONLINE_MOVETIME_FN)
	threechannel_query_list = get_3channel_move_query_list(THREECHANNEL_ONLINE_QUERY_FN, threechannel_online_movetime_list, loc_list)
	threechannel_err_distance_list = get_err_distance_list(threechannel_query_list, threechannel_db)
	plt_cdf(threechannel_err_distance_list, color_style[1])
