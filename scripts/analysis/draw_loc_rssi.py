...
def get_all_ap_from_db(db):
	all_ap = set()
	for db_entry in db:
		for ap in db_entry.keys():
			all_ap.add(ap)
	return all_ap

def get_all_ap_from_query_list(query_list):
	all_ap = set()
	for query_entry in query_list:
		for ap in query_entry.keys():
			all_ap.add(ap)
	return all_ap

def get_ap_rssilist_from_db(db):
	"""
	using all ap to reconstruct db, if ap not present in one location then set its rssi = -100
	"""
	all_ap = get_all_ap_from_db(db)
	ap_rssilist = {}
	for ap in all_ap:
		ap_rssilist[ap] = []
	...
	for db_entry in db:
		for ap in all_ap:
			if ap in db_entry:
				ap_rssilist[ap].append(db_entry[ap][0])
			else:
				ap_rssilist[ap].append(-100)
	return ap_rssilist

def get_ap_rssilist_from_query_list(query_list):
	"""
	using all ap to reconstruct db, if ap not present in one location then set its rssi = -100
	"""
	all_ap = get_all_ap_from_query_list(query_list)
	ap_rssilist = {}
	for ap in all_ap:
		ap_rssilist[ap] = []
	...
	for query_entry in query_list:
		for ap in all_ap:
			if ap in query_entry:
				ap_rssilist[ap].append(query_entry[ap])
			else:
				ap_rssilist[ap].append(-100)
	return ap_rssilist
...
def plt_loc_query_list(loc_list, query_list, colorstyle):
	...
	#for ap in get_all_ap_from_query_list(query_list):
	#	rssi_mean = np.mean(ap_rssilist[ap])
	#	if (rssi_mean > -75):
	#		print("ap:%s,rssi:%f;" % (ap, rssi_mean))
	#		plt.plot(loc_list, ap_rssilist[ap], '-', label=ap)
